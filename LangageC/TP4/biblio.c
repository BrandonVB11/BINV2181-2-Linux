...
bool lireFichier(struct Livre** bib,size_t taille_physique,size_t taille_logique, FILE* fichier){
   // on lit les livres 10 par 10
   struct Livre buffer[10];
   while (true){
      int nbreLus = fread(buffer, sizeof(struct Livre), 10, fichier);
      for(int i = 0; i < nbreLus; i++){
         bool succes = ajouterLivre(bib, buffer[i], taille_logique, taille_physique);
         if(!succes){
            perror("il y a eu un problème lors de l'ajout d'un livre");
            return false;
         }
      }
      if(nbreLus < 10){
         ...
         if(ferror(fichier)){
            perror("il y a eu une erreur lors de la lecture du fichier");
            fclose(fichier);
            return false;
         }
      }
